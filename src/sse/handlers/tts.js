...
async function handleSingleModelTts(body, modelStr, responseFormat, language) {
  ...
  if (!CREDENTIALED_PROVIDERS.has(provider)) {
    const result = await handleTtsCore({ provider, model, input: body.input, responseFormat, language, stability, languageCode });
  ...
  while (true) {
    ...
    const result = await handleTtsCore({ provider, model, input: body.input, credentials, responseFormat, language, stability, languageCode });
